...
interface OrgSettingsContextType {
  ...
  loading: boolean;
  updateSettings: (updates: Partial<OrganizationSettings>) => Promise<boolean>;
...
export function OrgSettingsProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <OrgSettingsContext.Provider value={{ settings, loading, updateSettings, uploadLogo, refetch: fetchSettings }}>
      {children}
...
export function useOrganizationSettings() {
  ...
  if (!ctx) throw new Error("useOrganizationSettings must be inside OrgSettingsProvider");
  return ctx;
}
